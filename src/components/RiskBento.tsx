...
const Card: React.FC<{ children: React.ReactNode; className?: string; delay?: number; variant?: 'red' | 'default' }> = ({ children, className, delay = 0, variant = 'default' }) => {
  return (
    <motion.div
      ...
      className={`relative group p-8 overflow-hidden rounded-xl backdrop-blur-sm border transition-all duration-500
        ${variant === 'red' 
    ...
    <section id="risk" className="py-32 px-6 max-w-7xl mx-auto">
      <div className="mb-20">
        <h2 className="font-display text-4xl md:text-5xl font-bold mb-6 tracking-tight">
          <span className="text-neon-red">Non-Compliance</span> is Not an Option.
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-6 h-auto md:h-[650px]">
        ...
        <Card className="md:col-span-2 md:row-span-2" delay={0.1} variant="red">
          <div className="flex justify-between items-start">
            <div className="p-3 bg-neon-red/20 rounded-lg border border-neon-red/20">
              <AlertTriangle className="w-6 h-6 text-neon-red" />
            </div>
            <span className="font-mono text-[10px] font-bold text-neon-red border border-neon-red/30 bg-neon-red/5 px-2 py-1 rounded">
              ESSENTIAL REQUIREMENTS
            </span>
          </div>
          <div className="mt-12">
            <h3 className="text-7xl md:text-8xl font-display font-bold text-white mb-4 tracking-tighter">
              €15M
            </h3>
        ...
        <Card className="md:col-span-1 md:row-span-1" delay={0.2}>
           <div className="flex justify-between items-start">
            ...
            <span className="font-mono text-[10px] text-slate-500">VIOLATION OF OBLIGATIONS</span>
          </div>
          <div className="mt-auto">
             <h3 className="text-5xl font-display font-bold text-white mb-2 tracking-tighter">€10M</h3>
             <p className="text-sm text-slate-400 font-medium">or 2% of Turnover</p>
        ...
        <Card className="md:col-span-1 md:row-span-1" delay={0.3}>
           <div className="flex justify-between items-start">
            ...
            <span className="font-mono text-[10px] text-slate-500">MISREPRESENTATION</span>
          </div>
          <div className="mt-auto">
             <h3 className="text-5xl font-display font-bold text-white mb-2 tracking-tighter">€5M</h3>
             <p className="text-sm text-slate-400 font-medium">or 1% of Turnover</p>
             <p className="mt-3 text-xs text-slate-500 leading-relaxed">
      ...
      {/* Bottom status bar */}
      <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-6 text-[10px] font-mono text-slate-600 tracking-wider">
