...
// logo is in public/logo.png, so we can access it directly as /logo.png
const logo = "/logo.png";
...
const CustomLogo: React.FC = () => (
    <img
        ...
        className="w-10 h-10 object-contain filter drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]"
    />
);
...
const Navigation: React.FC = () => {
    ...
    const navLinks = [
        { name: 'MUPP Engine', id: 'mupp' },
        { name: 'CRA-3D', id: 'cra3d' },
        { name: 'Sectors', id: 'sectors' },
        { name: 'Fines & Risk', id: 'risk' },
    ];

    // Custom Smooth Scroll Function (Restored)
    const smoothScrollTo = (e: React.MouseEvent, targetId: string, duration: number = 1500) => {
        e.preventDefault();
                ...
                <div className="hidden md:flex items-center gap-10">
                    {navLinks.map((link) => (
                        <a
                            key={link.name}
                            href={`#${link.id}`}
                            onClick={(e) => smoothScrollTo(e, link.id, 1200)}
